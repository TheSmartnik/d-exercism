...
struct TriangleType {
	static equilateral = "equilateral";
	static isosceles = "isosceles";
	static scalene = "scalene";
}

string kind(T...)(T args)
in {
	auto sides = [args];
	foreach(index, side; sides)
	{
		enforce(side > 0);
		enforce(sum(sides.dup.remove(index)) > side);
	}
} body {
	...
	check:
		for(;sides.length; sides = sides[1..$])
		{
			switch(count(sides, sides[0]))
			{
				case 3:
